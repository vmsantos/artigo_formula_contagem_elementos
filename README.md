...
- Volume de rejunte (com largura \(w\) e profundidade \(p\)):
  ...
  \]
**Exemplo**: Para uma parede com \(x = 10\) azulejos de \(0,3 \, \text{m}\), \(y = 8\) azulejos, \(w = 0,01 \, \text{m}\), \(p = 0,01 \, \text{m}\):
\[
...
\]
Adicionando 10% para desperdício: \(5,34 \times 1,1 \approx 5,87 \, \text{litros}\).
...
- Comprimento total de material: 
  ...
  L_{\text{total}} = L \cdot (2xy + x + y)
  \]
**Exemplo**: Para \(x = 10\), \(y = 8\), \(L = 2 \, \text{m}\):
\[
...
- Ajuste: \(x = \frac{X}{a}\), \(y = \frac{Y}{b}\), onde \(X\) e \(Y\) são as dimensões totais da estrutura.
- O comprimento total é then:
  \[
  L_{\text{total}} = a \cdot b \cdot (2 \cdot \frac{X}{a} \cdot \frac{Y}{b} + \frac{X}{a} + \frac{Y}{b}) = 2XY + aY + bX
  \]
